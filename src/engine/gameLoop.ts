import type {
  ...
  PlayerRoleRatingRecord, RegionId, InternationalTournament, TournamentSeed,
} from '../types';
import {
...
// ─── Qualification Logic & Tournament Builder ─────────────────────────────────

function getTeamAvgRating(state: GameState, teamId: string): number {
  const team = state.teams.get(teamId);
...
function getChampionsQualifiers(
  ...
  // Tiebreak: championsPoints → all-time wins in this league → mapDiff
...
export function awardPlayoffChampionsPoints(
  ...
  const place1  = bracket.champion;
  const place2  = gf.result.winner  === 'A' ? gf.teamBId   : gf.teamAId;
  const place3  = lf?.result  ? (lf.result.winner  === 'A' ? lf.teamBId   : lf.teamAId)  : null;
  const place4  = lr3?.result ? (lr3.result.winner === 'A' ? lr3.teamBId  : lr3.teamAId) : null;
  const place56 = [
    lr2a?.result ? (lr2a.result.winner === 'A' ? lr2a.teamBId : lr2a.teamAId) : null,
    lr2b?.result ? (lr2b.result.winner === 'A' ? lr2b.teamBId : lr2b.teamAId) : null,
  ].filter(Boolean) as string[];

  const award = (teamId: string | null | undefined, pts: number) => {
    if (!teamId) return;
  ...
  if (isSplit3) {
    // Seeds 1 & 2 auto-qualify — no placement points; 3rd and 4th still earn
    award(place3, 4);
    award(place4, 3);
  } else {
    award(place1, 5);
  ...
  void place56; // 5th/6th earn no regional playoff points
}
...
export function awardTournamentChampionsPoints(
  ...
  const isMasters1 = tournament.name === 'Masters 1';
