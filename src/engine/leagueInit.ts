...
function assignRoster(
  ...
  roster.forEach(p => {
    ...
    const calYears = randInt(rng, 1, 3); // 1–3 calendar years
    const calYear = Math.ceil(season / 3);
    contracts.set(contractId, {
      ...
      startSeason: season,
      endSeason: (calYear + calYears - 1) * 3,
    });
  });
...
export function initLeague(regionId: RegionId, rng: Rng): LeagueInitResult {
  ...
  partnershipByPrestige.forEach((team, rank) => {
    ...
    head.teamId = team.id;
    head.role = 'head';
    head.contractEndSeason = (1 + randInt(rng, 0, 2)) * 3;
    ...
    if (rank < 8) {
      ...
      asst.teamId = team.id;
      asst.role = 'assistant';
      asst.contractEndSeason = (1 + randInt(rng, 0, 2)) * 3;
